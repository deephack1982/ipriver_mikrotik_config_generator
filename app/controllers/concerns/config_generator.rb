module ConfigGenerator
  def generate_rsc(config)
    ...
    if @config.interfaces.first.interface_type == 'ethernet-wan' || @config.interfaces.first.interface_type == 'sfp-wan'
      ...
      @output << "add action=drop chain=forward comment=\"defconf:  drop all from WAN not DSTNATed\" connection-nat-state=!dstnat connection-state=new in-interface=ether1\n"
